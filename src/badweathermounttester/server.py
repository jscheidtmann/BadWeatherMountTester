...
class WebServer:
    ...
    def _setup_routes(self) -> None:
        ...
        def simulation_setup():
            ...
            # Direct velocity override: use the entered px/s value as-is
            if self.config.velocity.sim_velocity_override_px_s is not None:
                override_px_s = self.config.velocity.sim_velocity_override_px_s
                if override_px_s != 0:
                    pixels_per_second = max(0.001, override_px_s)
                    velocity_profile = None
                    velocity_source = "velocity_override"
            ...
            return jsonify(
                {
                    ...
                    "sim_velocity_override_px_s": self.config.velocity.sim_velocity_override_px_s,
                }
            )

        @self.app.route("/api/simulation/velocity", methods=["POST"])
        def set_drift():
            """Set the simulation velocity override in simulator px/s."""
            data = request.get_json()
            if not data or "sim_px_per_s" not in data:
                return jsonify({"error": "sim_px_per_s required"}), 400
            sim_px_s = float(data["sim_px_per_s"])
            if sim_px_s == 0:
                return jsonify({"error": "sim_px_per_s must be non-zero"}), 400
            self.config.velocity.sim_velocity_override_px_s = sim_px_s
            self.config.save_yaml(self.setup_path)
            log_vel.info("Simulation velocity override set to %.4f sim px/s", sim_px_s)
            return jsonify({"status": "ok", "sim_px_per_s": round(sim_px_s, 4)})

        @self.app.route("/api/simulation/velocity", methods=["DELETE"])
        def clear_drift():
            """Clear the simulation velocity override."""
            self.config.velocity.sim_velocity_override_px_s = None
            self.config.save_yaml(self.setup_path)
            log_vel.info("Simulation velocity override cleared")
            return jsonify({"status": "ok"})
